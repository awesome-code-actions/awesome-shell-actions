...
alias ks=k-switch
function k-switch {
  local cfg=$(find -L ~/.kube/ -maxdepth 1 -type f | fzf)
  export KUBECONFIG=$cfg
}

function kubectl-load-vagrant {
  local cfg=$(vagrant-list | fzf)
  local name=$(echo $cfg | awk '{print $1}')
  local ip=$(echo $cfg | awk '{print $2}')
  echo $name $ip
  scp root@$ip:/etc/kubernetes/admin.conf ~/.kube/vagrant-$name
}

function kubectl-curl {
  local cfg=$1
  local curl_arg=$2
  export clientcert=$(grep client-cert $cfg | cut -d" " -f 6)
  # echo $clientcert

  export clientkey=$(grep client-key-data $cfg | cut -d" " -f 6)
  # echo $clientkey

  export certauth=$(grep certificate-authority-data $cfg | cut -d" " -f 6)
  # echo $certauth

  echo $clientcert | base64 -d >./client.pem
  echo $clientkey | base64 -d >./client-key.pem
  echo $certauth | base64 -d >./ca.pem
  echo curl --cert ./client.pem --key ./client-key.pem --cacert ./ca.pem $curl_arg
  curl --cert ./client.pem --key ./client-key.pem --cacert ./ca.pem $curl_arg
}

function kubectl-fetch-logs() {
  local ns=$1
  local name=$2
  while read po; do
    echo "$po"
    kubectl logs -n $ns $po >$po.log
